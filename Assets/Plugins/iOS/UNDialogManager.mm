...
extern "C" {
    ...
    int _showAlertPromptDialog(const char *title, const char *msg, const char *pBtn, const char *nBtn){
        
        return [[UNDialogManager sharedManager]
                showPromptDialog:[NSString stringWithUTF8String:title]
                message:[NSString stringWithUTF8String:msg]
                positiveButton:[NSString stringWithUTF8String:pBtn]
                negativeButton:[NSString stringWithUTF8String:nBtn]];
    }
...
- (int) showPromptDialog:(NSString *)title message:(NSString*)msg positiveButton:(NSString*)pButton negativeButton:(NSString*)nButton{
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:title message:msg delegate:self cancelButtonTitle:nButton otherButtonTitles:pButton, nil];
    alert.tag = ++_id;
...
- (void)alertView:(UIAlertView*)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    bool isTextInput = (alertView.alertViewStyle == UIAlertViewStylePlainTextInput);
    
    switch (buttonIndex) {
        case 0:
            if(alertView.cancelButtonIndex == 0) {
                if(isTextInput){
                    UnitySendMessage("DialogManager", "OnPromptCancel", "");
                }
                else{
                ...
                if(isTextInput){
                    ...
                    UnitySendMessage("DialogManager", "OnPromptDone", input.UTF8String);
                }
                else{UnitySendMessage("DialogManager", "OnSubmit", tag.UTF8String);}
            }
            break;
        case 1:
            if(alertView.cancelButtonIndex == 1) {
                ...
                else{UnitySendMessage("DialogManager", "OnCancel", tag.UTF8String);}
            }
            else {
                if(isTextInput){
                    NSString *input =[[alertView textFieldAtIndex:0] text];
