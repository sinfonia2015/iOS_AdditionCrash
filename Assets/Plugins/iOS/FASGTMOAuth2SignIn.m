...
@interface FASGTMOAuth2SignIn ()
@property (assign) BOOL hasHandledCallback;
@property (retain) FASGTMOAuth2Fetcher *pendingFetcher;
...
finishedWithFetcher:(FASGTMOAuth2Fetcher *)fetcher
       error:(NSError *)error;

#if !FASGTM_OAUTH2_SKIP_GOOGLE_SUPPORT
- (void)infoFetcher:(FASGTMOAuth2Fetcher *)fetcher
...
@implementation FASGTMOAuth2SignIn
...
#if FASGTM_OAUTH2_USES_OPENIDCONNECT
...
- (void)addScopeForGoogleUserInfo {
...
#endif  // FASGTM_OAUTH2_USES_OPENIDCONNECT

  FASGTMOAuth2Authentication *auth = self.authentication;
  if (self.shouldFetchGoogleUserEmail) {
    NSString *scopeStrings = auth.scope;
    if (!hasScope(scopeStrings, emailScope)) {
      scopeStrings = [FASGTMOAuth2Authentication scopeWithStrings:scopeStrings, emailScope, nil];
  ...
  if (self.shouldFetchGoogleUserProfile) {
    ...
    if (!hasScope(scopeStrings, profileScope)) {
      scopeStrings = [FASGTMOAuth2Authentication scopeWithStrings:scopeStrings, profileScope, nil];
      auth.scope = scopeStrings;
    }
  }
}
...
- (id)initWithAuthentication:(FASGTMOAuth2Authentication *)auth
  ...
  FASGTMOAuth2AssertValidSelector(delegate, webRequestSelector,
    @encode(FASGTMOAuth2SignIn *), @encode(NSURLRequest *), 0);
  FASGTMOAuth2AssertValidSelector(delegate, finishedSelector,
    @encode(FASGTMOAuth2SignIn *), @encode(FASGTMOAuth2Authentication *),
    @encode(NSError *), 0);
...
#if DEBUG
    NSAssert(hasClientID, @"FASGTMOAuth2SignIn: clientID needed");
    NSAssert(hasRedirect, @"FASGTMOAuth2SignIn: redirectURI needed");
#endif
    return nil;
  }
...
- (BOOL)startWebRequest {
  ...
  NSString *paramStr = [FASGTMOAuth2Authentication encodedQueryParametersForDictionary:paramsDict];
...
- (void)windowWasClosed {
  ...
  NSError *error = [NSError errorWithDomain:kFASGTMOAuth2ErrorDomain
                                       code:kFASGTMOAuth2ErrorWindowClosed
                                   userInfo:nil];
...
- (BOOL)titleChanged:(NSString *)title {
  ...
  if (spaceIndex != NSNotFound) {
    ...
    NSDictionary *dict = [FASGTMOAuth2Authentication dictionaryWithResponseString:responseStr];
...
- (void)authCodeObtained {
  ...
  if ([code length] > 0) {
    ...
    FASGTMOAuth2Fetcher *fetcher = [auth beginTokenFetchWithDelegate:self
                                                didFinishSelector:sel];
    if (fetcher == nil) {
      error = [NSError errorWithDomain:kFASGTMOAuth2FetcherStatusDomain
                                  code:-1
                              userInfo:nil];
    } else {
    ...
    [nc postNotificationName:kFASGTMOAuth2UserSignedIn
                      object:self
                    userInfo:nil];
  } else {
    ...
    if ([errStr length] > 0) {
      userInfo = [NSDictionary dictionaryWithObject:errStr
                                             forKey:kFASGTMOAuth2ErrorMessageKey];
    }

    error = [NSError errorWithDomain:kFASGTMOAuth2ErrorDomain
                                code:kFASGTMOAuth2ErrorAuthorizationFailed
                            userInfo:userInfo];
  }
...
finishedWithFetcher:(FASGTMOAuth2Fetcher *)fetcher
  ...
  if (error == nil
      ...
      && [self.authentication.serviceProvider isEqual:kFASGTMOAuth2ServiceProviderGoogle]) {
...
+ (FASGTMOAuth2Fetcher *)userInfoFetcherWithAuth:(FASGTMOAuth2Authentication *)auth {
  ...
  id <FASGTMOAuth2FetcherServiceProtocol> fetcherService = nil;
  ...
  [fetcher setCommentWithFormat:@"FASGTMOAuth2 user info %@", forStr];
#endif
  return fetcher;
}
...
- (void)fetchGoogleUserInfo {
  ...
  FASGTMOAuth2Authentication *auth = self.authentication;
  FASGTMOAuth2Fetcher *fetcher = [[self class] userInfoFetcherWithAuth:auth];
  ...
  [auth notifyFetchIsRunning:YES
                     fetcher:fetcher
                        type:kFASGTMOAuth2FetchTypeUserInfo];
}
...
static void ReachabilityCallBack(SCNetworkReachabilityRef target,
  ...
  FASGTMOAuth2SignIn *signIn = (FASGTMOAuth2SignIn *)info;
...
- (void)reachabilityTarget:(SCNetworkReachabilityRef)reachabilityRef
  ...
  if (isConnected) {
    ...
    if (hasNotifiedNetworkLoss_) {
      ...
      NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
      [nc postNotificationName:kFASGTMOAuth2NetworkFound
                        object:self
                      userInfo:nil];
...
- (void)reachabilityTimerFired:(NSTimer *)timer {
  ...
  if (!hasNotifiedNetworkLoss_) {
    NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
    [nc postNotificationName:kFASGTMOAuth2NetworkLost
                      object:self
                    userInfo:nil];
...
+ (void)revokeTokenForGoogleAuthentication:(FASGTMOAuth2Authentication *)auth {
  if (auth.refreshToken != nil
      && auth.canAuthorize
      && [auth.serviceProvider isEqual:kFASGTMOAuth2ServiceProviderGoogle]) {
    ...
    NSString *encoded = [FASGTMOAuth2Authentication encodedOAuthValueForString:token];
    if (encoded != nil) {
      ...
      FASGTMOAuth2Fetcher *fetcher;
      id <FASGTMOAuth2FetcherServiceProtocol> fetcherService = auth.fetcherService;
      if (fetcherService) {
        fetcher = (FASGTMOAuth2Fetcher *)[fetcherService fetcherWithRequest:request];
      } else {
        fetcher = [FASGTMOAuth2Fetcher fetcherWithRequest:request];
      }
      [fetcher setCommentWithFormat:@"FASGTMOAuth2 revoke token for %@", auth.userEmail];
