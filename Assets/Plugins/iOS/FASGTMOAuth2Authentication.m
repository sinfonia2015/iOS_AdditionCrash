...
NSString *const kFASGTMOAuth2ServiceProviderGoogle = @"Google";

NSString *const kFASGTMOAuth2ErrorDomain  = @"com.google.FASGTMOAuth2";

NSString *const kFASGTMOAuth2ErrorMessageKey = @"error";
NSString *const kFASGTMOAuth2ErrorRequestKey = @"request";
NSString *const kFASGTMOAuth2ErrorJSONKey    = @"json";

// Notifications
NSString *const kFASGTMOAuth2FetchStarted        = @"kFASGTMOAuth2FetchStarted";
NSString *const kFASGTMOAuth2FetchStopped        = @"kFASGTMOAuth2FetchStopped";

NSString *const kFASGTMOAuth2FetcherKey          = @"fetcher";
NSString *const kFASGTMOAuth2FetchTypeKey        = @"FetchType";
NSString *const kFASGTMOAuth2FetchTypeToken      = @"token";
NSString *const kFASGTMOAuth2FetchTypeRefresh    = @"refresh";
NSString *const kFASGTMOAuth2FetchTypeAssertion  = @"assertion";
NSString *const kFASGTMOAuth2FetchTypeUserInfo   = @"userInfo";

NSString *const kFASGTMOAuth2ErrorKey                  = @"error";
NSString *const kFASGTMOAuth2ErrorObjectKey            = @"kFASGTMOAuth2ErrorObjectKey";

NSString *const kFASGTMOAuth2ErrorInvalidRequest       = @"invalid_request";
NSString *const kFASGTMOAuth2ErrorInvalidClient        = @"invalid_client";
NSString *const kFASGTMOAuth2ErrorInvalidGrant         = @"invalid_grant";
NSString *const kFASGTMOAuth2ErrorUnauthorizedClient   = @"unauthorized_client";
NSString *const kFASGTMOAuth2ErrorUnsupportedGrantType = @"unsupported_grant_type";
NSString *const kFASGTMOAuth2ErrorInvalidScope         = @"invalid_scope";

NSString *const kFASGTMOAuth2UserSignedIn              = @"kFASGTMOAuth2UserSignedIn";

NSString *const kFASGTMOAuth2AccessTokenRefreshed     = @"kFASGTMOAuth2AccessTokenRefreshed";
NSString *const kFASGTMOAuth2RefreshTokenChanged      = @"kFASGTMOAuth2RefreshTokenChanged";
NSString *const kFASGTMOAuth2AccessTokenRefreshFailed = @"kFASGTMOAuth2AccessTokenRefreshFailed";

NSString *const kFASGTMOAuth2WebViewStartedLoading = @"kFASGTMOAuth2WebViewStartedLoading";
NSString *const kFASGTMOAuth2WebViewStoppedLoading = @"kFASGTMOAuth2WebViewStoppedLoading";
NSString *const kFASGTMOAuth2WebViewKey            = @"kFASGTMOAuth2WebViewKey";
NSString *const kFASGTMOAuth2WebViewStopKindKey    = @"kFASGTMOAuth2WebViewStopKindKey";
NSString *const kFASGTMOAuth2WebViewFinished       = @"finished";
NSString *const kFASGTMOAuth2WebViewFailed         = @"failed";
NSString *const kFASGTMOAuth2WebViewCancelled      = @"cancelled";

NSString *const kFASGTMOAuth2NetworkLost         = @"kFASGTMOAuthNetworkLost";
NSString *const kFASGTMOAuth2NetworkFound        = @"kFASGTMOAuthNetworkFound";
...
// If FASGTMNSJSONSerialization is available, it is used for formatting JSON
...
+ (FASGTMOAuth2AuthorizationArgs *)argsWithRequest:(NSMutableURLRequest *)req
...
@implementation FASGTMOAuth2AuthorizationArgs
...
+ (FASGTMOAuth2AuthorizationArgs *)argsWithRequest:(NSMutableURLRequest *)req
                              ...
                              completionHandler:(id)completionHandler
                                         thread:(NSThread *)thread {
  FASGTMOAuth2AuthorizationArgs *obj;
  obj = [[[FASGTMOAuth2AuthorizationArgs alloc] init] autorelease];
  obj.request = req;
...
@interface FASGTMOAuth2Authentication ()
...
- (BOOL)authorizeRequestArgs:(FASGTMOAuth2AuthorizationArgs *)args;

- (BOOL)authorizeRequestImmediateArgs:(FASGTMOAuth2AuthorizationArgs *)args;

- (BOOL)shouldRefreshAccessToken;

- (void)updateExpirationDate;

- (void)tokenFetcher:(FASGTMOAuth2Fetcher *)fetcher
    finishedWithData:(NSData *)data
               error:(NSError *)error;

- (void)auth:(FASGTMOAuth2Authentication *)auth
finishedRefreshWithFetcher:(FASGTMOAuth2Fetcher *)fetcher
       error:(NSError *)error;

- (void)invokeCallbackArgs:(FASGTMOAuth2AuthorizationArgs *)args;
...
@implementation FASGTMOAuth2Authentication
...
+ (id)authenticationWithServiceProvider:(NSString *)serviceProvider
  ...
  FASGTMOAuth2Authentication *obj = [[[self alloc] init] autorelease];
...
- (void)setKeysForResponseDictionary:(NSDictionary *)dict {
  ...
  if (didRefreshTokenChange) {
    ...
    [nc postNotificationName:kFASGTMOAuth2RefreshTokenChanged
                      object:self
                    userInfo:nil];
  }
  // NSLog(@"keys set ----------------------------\n%@", dict);
...
+ (NSDictionary *)dictionaryWithJSONData:(NSData *)data {
  ...
  if (serializer) {
    ...
    if (jsonParseClass) {
      FASGTMOAuth2ParserClass *parser = [[[jsonParseClass alloc] init] autorelease];
...
#if DEBUG
      NSAssert(0, @"FASGTMOAuth2Authentication: No parser available");
#endif
    }
  }
  return nil;
...
- (void)authorizeRequest:(NSMutableURLRequest *)request
  ...
  FASGTMOAuth2AuthorizationArgs *args;
  args = [FASGTMOAuth2AuthorizationArgs argsWithRequest:request
                                            delegate:nil
                                            selector:NULL
...
//   - (void)authentication:(FASGTMOAuth2Authentication *)auth
//                  request:(NSMutableURLRequest *)request
//        finishedWithError:(NSError *)error;
- (void)authorizeRequest:(NSMutableURLRequest *)request
                delegate:(id)delegate
       didFinishSelector:(SEL)sel {
  FASGTMOAuth2AssertValidSelector(delegate, sel,
                               @encode(FASGTMOAuth2Authentication *),
...
finishedRefreshWithFetcher:(FASGTMOAuth2Fetcher *)fetcher
  ...
  @synchronized(authorizationQueue_) {
    ...
    if (hasAccessToken && error == nil) {
      // Successful refresh.
      noteName = kFASGTMOAuth2AccessTokenRefreshed;
      userInfo = nil;
    } else {
      ...
      noteName = kFASGTMOAuth2AccessTokenRefreshFailed;
      NSString *jsonErr = nil;
      if ([error code] == kFASGTMOAuth2StatusBadRequest) {
        NSDictionary *json = [[error userInfo] objectForKey:kFASGTMOAuth2ErrorJSONKey];
        jsonErr = [json objectForKey:kFASGTMOAuth2ErrorMessageKey];
      }
      // error and jsonErr may be nil
      userInfo = [NSMutableDictionary dictionary];
      [userInfo setValue:error forKey:kFASGTMOAuth2ErrorObjectKey];
      [userInfo setValue:jsonErr forKey:kFASGTMOAuth2ErrorMessageKey];
    }
    NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
    ...
    for (FASGTMOAuth2AuthorizationArgs *args in pendingAuthQueue) {
...
- (BOOL)isAuthorizingRequest:(NSURLRequest *)request {
  BOOL wasFound = NO;
  @synchronized(authorizationQueue_) {
    for (FASGTMOAuth2AuthorizationArgs *args in authorizationQueue_) {
  ...
  if (isAuthorizableRequest && [accessToken length] > 0) {
    if (request) {
      ...
      NSString *value = [NSString stringWithFormat:@"%s %@",
                         FASGTM_OAUTH2_BEARER, accessToken];
    ...
    if (request) {
      userInfo = [NSDictionary dictionaryWithObject:request
                                             forKey:kFASGTMOAuth2ErrorRequestKey];
    }
    NSInteger code = (isAuthorizableRequest ?
                      kFASGTMOAuth2ErrorAuthorizationFailed :
                      kFASGTMOAuth2ErrorUnauthorizableRequest);
    args.error = [NSError errorWithDomain:kFASGTMOAuth2ErrorDomain
                                     code:code
                                 userInfo:userInfo];
  }
...
- (void)waitForCompletionWithTimeout:(NSTimeInterval)timeoutInSeconds {
  ...
  FASGTMOAuth2Fetcher *fetcher = self.refreshFetcher;
...
- (NSString *)userAgent {
  ...
  NSString *userAgent = @"FASGTM-oauth2";
  if (appID) {
...
- (FASGTMOAuth2Fetcher *)beginTokenFetchWithDelegate:(id)delegate
  ...
  if (refreshToken) {
    ...
    fetchType = kFASGTMOAuth2FetchTypeRefresh;
  } else if (code) {
    ...
    fetchType = kFASGTMOAuth2FetchTypeToken;
  } else if (assertion) {
    ...
    fetchType = kFASGTMOAuth2FetchTypeAssertion;
  } else {
#if DEBUG
 ...
 forHTTPHeaderField:@"Content-Type"];
  ...
  FASGTMOAuth2Fetcher *fetcher;
  id <FASGTMOAuth2FetcherServiceProtocol> fetcherService = self.fetcherService;
  if (fetcherService) {
    fetcher = (FASGTMOAuth2Fetcher *)[fetcherService fetcherWithRequest:request];

    // Don't use an authorizer for an auth token fetch
    fetcher.authorizer = nil;
  } else {
    fetcher = [FASGTMOAuth2Fetcher fetcherWithRequest:request];
  }
  ...
  [fetcher setCommentWithFormat:@"FASGTMOAuth2 %@ fetch to %@ %@", fetchType, [tokenURL host], forStr];
#endif
  ...
  if (error) {
      ...
      if (isResponseJSON) {
        ...
        if ([errorJson count] > 0) {
          ...
          userInfo = [NSMutableDictionary dictionaryWithObject:errorJson
                                                        forKey:kFASGTMOAuth2ErrorJSONKey];
    ...
    if (hasData) {
      ...
      if (tokenType == nil
        ...
        NSLog(@"FASGTMOAuth2: Unexpected token type: %@", tokenType);
      }
#endif
    }
  }
...
- (void)notifyFetchIsRunning:(BOOL)isStarting
                     fetcher:(FASGTMOAuth2Fetcher *)fetcher
                        type:(NSString *)fetchType {
  NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];

  NSString *name = (isStarting ? kFASGTMOAuth2FetchStarted : kFASGTMOAuth2FetchStopped);
  NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:
                        fetcher, kFASGTMOAuth2FetcherKey,
                        fetchType, kFASGTMOAuth2FetchTypeKey, // fetchType may be nil
                        nil];
...
#endif // FASGTM_INCLUDE_OAUTH2 || !GDATA_REQUIRE_SERVICE_INCLUDES
