...
#import "FASGTMOAuth2SignIn.h"
#import "FASGTMOAuth2Authentication.h"

NSString *const kFASGTMOAuth2KeychainErrorDomain = @"com.google.FASGTMOAuthKeychain";

NSString *const kFASGTMOAuth2CookiesWillSwapOut = @"kFASGTMOAuth2CookiesWillSwapOut";
NSString *const kFASGTMOAuth2CookiesDidSwapIn   = @"kFASGTMOAuth2CookiesDidSwapIn";

static NSString * const kFASGTMOAuth2AccountName = @"OAuth";
static FASGTMOAuth2Keychain* gFASGTMOAuth2DefaultKeychain = nil;

@interface FASGTMOAuth2ViewControllerTouch()
@property (nonatomic, copy) NSURLRequest *request;
@property (nonatomic, copy) NSArray *systemCookies;
@property (nonatomic, copy) NSArray *signInCookies;
@end

@implementation FASGTMOAuth2ViewControllerTouch
...
- (id)initWithScope:(NSString *)scope
  ...
  FASGTMOAuth2Authentication *auth;
...
+ (id)controllerWithScope:(NSString *)scope
             ...
             clientSecret:(NSString *)clientSecret
         keychainItemName:(NSString *)keychainItemName
        completionHandler:(FASGTMOAuth2ViewControllerCompletionHandler)handler {
  return [[[self alloc] initWithScope:scope
                             clientID:clientID
                         clientSecret:clientSecret
                     keychainItemName:keychainItemName
...
#endif // !FASGTM_OAUTH2_SKIP_GOOGLE_SUPPORT

+ (id)controllerWithAuthentication:(FASGTMOAuth2Authentication *)auth
...
#if !FASGTM_OAUTH2_SKIP_GOOGLE_SUPPORT
+ (FASGTMOAuth2Authentication *)authForGoogleFromKeychainForName:(NSString *)keychainItemName
                                                     clientID:(NSString *)clientID
                                                 clientSecret:(NSString *)clientSecret {
...
+ (FASGTMOAuth2Authentication *)authForGoogleFromKeychainForName:(NSString *)keychainItemName
  ...
  FASGTMOAuth2Authentication *auth;
  auth = [FASGTMOAuth2Authentication authenticationWithServiceProvider:kFASGTMOAuth2ServiceProviderGoogle
                                                           tokenURL:tokenURL
...
+ (BOOL)authorizeFromKeychainForName:(NSString *)keychainItemName
                      authentication:(FASGTMOAuth2Authentication *)newAuth
                               error:(NSError **)error {
  newAuth.accessToken = nil;

  BOOL didGetTokens = NO;
  FASGTMOAuth2Keychain *keychain = [FASGTMOAuth2Keychain defaultKeychain];
  NSString *password = [keychain passwordForService:keychainItemName
                                            account:kFASGTMOAuth2AccountName
                                              error:error];
  if (password != nil) {
...
+ (BOOL)saveParamsToKeychainForName:(NSString *)keychainItemName
                     authentication:(FASGTMOAuth2Authentication *)auth {
...
+ (BOOL)saveParamsToKeychainForName:(NSString *)keychainItemName
  ...
  if (![auth canAuthorize]) {
    if (error) {
      *error = [NSError errorWithDomain:kFASGTMOAuth2ErrorDomain
                                   code:kFASGTMOAuth2ErrorTokenUnavailable
                               userInfo:nil];
    }
    return NO;
  }
...
- (void)notifyWithName:(NSString *)name
               webView:(UIWebView *)webView
                  kind:(NSString *)kind {
  BOOL isStarting = [name isEqual:kFASGTMOAuth2WebViewStartedLoading];
  ...
  NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:
                        webView, kFASGTMOAuth2WebViewKey,
                        kind, kFASGTMOAuth2WebViewStopKindKey, // kind may be nil
                        nil];
...
+ (Class)signInClass {
  if (gSignInClass == Nil) {
    gSignInClass = [FASGTMOAuth2SignIn class];
  }
  return gSignInClass;
}
...
- (FASGTMOAuth2Authentication *)authentication {
...
- (void)signIn:(FASGTMOAuth2SignIn *)signIn displayRequest:(NSURLRequest *)request {
...
- (void)signIn:(FASGTMOAuth2SignIn *)signIn
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:kFASGTMOAuth2CookiesWillSwapOut
                                                      object:self
                                                    userInfo:nil];
  [self swapOutCookies];
...
- (void)viewWillDisappear:(BOOL)animated {
  if (![self isBeingObscured:self]) {
    if (!didDismissSelf_) {
      ...
      [self notifyWithName:kFASGTMOAuth2WebViewStoppedLoading
                   webView:self.webView
                      kind:kFASGTMOAuth2WebViewCancelled];
  ...
  [self swapInCookies];
  [[NSNotificationCenter defaultCenter] postNotificationName:kFASGTMOAuth2CookiesDidSwapIn
                                                      object:self
                                                    userInfo:nil];
...
- (void)webViewDidStartLoad:(UIWebView *)webView {
  [self notifyWithName:kFASGTMOAuth2WebViewStartedLoading
               webView:webView
                  kind:nil];
  [self updateUI];
}
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
  [self notifyWithName:kFASGTMOAuth2WebViewStoppedLoading
               webView:webView
                  kind:kFASGTMOAuth2WebViewFinished];
  ...
  if ([title length] > 0) {
    ...
    NSAssert([result integerValue] == 2, @"FASGTMOAuth2: Javascript is required");
...
- (void)webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error {
  [self notifyWithName:kFASGTMOAuth2WebViewStoppedLoading
               webView:webView
                  kind:kFASGTMOAuth2WebViewFailed];
...
@implementation FASGTMOAuth2Keychain

+ (FASGTMOAuth2Keychain *)defaultKeychain {
  if (gFASGTMOAuth2DefaultKeychain == nil) {
    gFASGTMOAuth2DefaultKeychain = [[self alloc] init];
  }
  return gFASGTMOAuth2DefaultKeychain;
}


// For unit tests: allow setting a mock object
+ (void)setDefaultKeychain:(FASGTMOAuth2Keychain *)keychain {
  if (gFASGTMOAuth2DefaultKeychain != keychain) {
    [gFASGTMOAuth2DefaultKeychain release];
    gFASGTMOAuth2DefaultKeychain = [keychain retain];
  }
}

- (NSString *)keyForService:(NSString *)service account:(NSString *)account {
  return [NSString stringWithFormat:@"com.google.FASGTMOAuth.%@%@", service, account];
}
...
- (NSString *)passwordForService:(NSString *)service account:(NSString *)account error:(NSError **)error {
  ...
  if (0 < [service length] && 0 < [account length]) {
    ...
    if (result == nil && error != NULL) {
      *error = [NSError errorWithDomain:kFASGTMOAuth2KeychainErrorDomain
                                   code:kFASGTMOAuth2KeychainErrorNoPassword
                               userInfo:nil];
    }
  } else if (error != NULL) {
    *error = [NSError errorWithDomain:kFASGTMOAuth2KeychainErrorDomain
                                 code:kFASGTMOAuth2KeychainErrorBadArguments
                             userInfo:nil];
  }
  return result;
...
// iPhone
- (NSString *)passwordForService:(NSString *)service account:(NSString *)account error:(NSError **)error {
  OSStatus status = kFASGTMOAuth2KeychainErrorBadArguments;
  NSString *result = nil;
  if (0 < [service length] && 0 < [account length]) {
