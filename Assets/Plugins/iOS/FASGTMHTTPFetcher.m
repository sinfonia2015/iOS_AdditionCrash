...
static id <FASGTMCookieStorageProtocol> gFASGTMFetcherStaticCookieStorage = nil;
static Class gFASGTMFetcherConnectionClass = nil;


NSString *const kFASGTMHTTPFetcherStartedNotification           = @"kFASGTMHTTPFetcherStartedNotification";
NSString *const kFASGTMHTTPFetcherStoppedNotification           = @"kFASGTMHTTPFetcherStoppedNotification";
NSString *const kFASGTMHTTPFetcherRetryDelayStartedNotification = @"kFASGTMHTTPFetcherRetryDelayStartedNotification";
NSString *const kFASGTMHTTPFetcherRetryDelayStoppedNotification = @"kFASGTMHTTPFetcherRetryDelayStoppedNotification";

NSString *const kFASGTMHTTPFetcherErrorDomain       = @"com.google.FASGTMHTTPFetcher";
NSString *const kFASGTMHTTPFetcherStatusDomain      = @"com.google.HTTPStatus";
NSString *const kFASGTMHTTPFetcherErrorChallengeKey = @"challenge";
NSString *const kFASGTMHTTPFetcherStatusDataKey     = @"data";  // data returned with a kFASGTMHTTPFetcherStatusDomain error
...
@interface FASGTMHTTPFetcher ()

@property (copy) NSString *temporaryDownloadPath;
@property (retain) id <FASGTMCookieStorageProtocol> cookieStorage;
...
- (void)authorizer:(id <FASGTMFetcherAuthorizationProtocol>)auth
...
@interface FASGTMHTTPFetcher (FASGTMHTTPFetcherLoggingInternal)
- (void)setupStreamLogging;
- (void)logFetchWithError:(NSError *)error;
- (void)logNowWithError:(NSError *)error;
@end

@implementation FASGTMHTTPFetcher
...
+ (void)initialize {
  ...
  // thread-safe manner
  if (!gFASGTMFetcherStaticCookieStorage) {
    Class cookieStorageClass = NSClassFromString(@"FASGTMCookieStorage");
    if (cookieStorageClass) {
      gFASGTMFetcherStaticCookieStorage = [[cookieStorageClass alloc] init];
    }
  }
}
...
- (id)initWithRequest:(NSURLRequest *)request {
  ...
  if (self) {
    ...
    if (gFASGTMFetcherStaticCookieStorage != nil) {
      ...
      [self setCookieStorageMethod:kFASGTMHTTPFetcherCookieStorageMethodStatic];
    } else {
      // Default to system default cookie storage
      [self setCookieStorageMethod:kFASGTMHTTPFetcherCookieStorageMethodSystemDefault];
    }
...
- (BOOL)beginFetchWithDelegate:(id)delegate
  ...
  FASGTMAssertSelectorNilOrImplementedWithArgs(delegate, finishedSelector, @encode(FASGTMHTTPFetcher *), @encode(NSData *), @encode(NSError *), 0);
  FASGTMAssertSelectorNilOrImplementedWithArgs(delegate, receivedDataSel_, @encode(FASGTMHTTPFetcher *), @encode(NSData *), 0);
  FASGTMAssertSelectorNilOrImplementedWithArgs(delegate, retrySel_, @encode(FASGTMHTTPFetcher *), @encode(BOOL), @encode(NSError *), 0);
...
- (BOOL)beginFetchMayDelay:(BOOL)mayDelay
...
#if !FASGTM_ALLOW_INSECURE_REQUESTS
  if (requestURL != nil) {
...
#endif  // FASGTM_ALLOW_INSECURE_REQUESTS
  ...
  if (isPreIOS6 && delegateQueue) {
    NSLog(@"FASGTMHTTPFetcher delegateQueue not safe in iOS 5");
  }
#endif
  ...
  [defaultNC postNotificationName:kFASGTMHTTPFetcherStartedNotification
                           object:self];
  return YES;
...
- (void)failToBeginFetchWithError:(NSError *)error {
  if (error == nil) {
    error = [NSError errorWithDomain:kFASGTMHTTPFetcherErrorDomain
                                code:kFASGTMHTTPFetcherErrorDownloadFailed
                            userInfo:nil];
  }
...
- (void)backgroundFetchExpired {
  ...
  NSError *error = [NSError errorWithDomain:kFASGTMHTTPFetcherErrorDomain
                                       code:kFASGTMHTTPFetcherErrorBackgroundExpiration
                                   userInfo:nil];
  [self invokeFetchCallbacksOnDelegateQueueWithData:nil
                                              error:error];
  @synchronized(self) {
...
#endif // FASGTM_BACKGROUND_FETCHING
...
- (NSString *)createTempDownloadFilePathForPath:(NSString *)targetPath {
  ...
  NSString *name = [NSString stringWithFormat:@"FASGTMhttpfetcher_%u_%u",
...
- (void)addCookiesToRequest:(NSMutableURLRequest *)request {
  ...
  if (cookieStorageMethod_ != kFASGTMHTTPFetcherCookieStorageMethodSystemDefault
      && cookieStorageMethod_ != kFASGTMHTTPFetcherCookieStorageMethodNone) {
...
- (void)stopFetchReleasingCallbacks:(BOOL)shouldReleaseCallbacks {
  id <FASGTMHTTPFetcherServiceProtocol> service;
...
- (void)sendStopNotificationIfNeeded {
  ...
  if (sendNow) {
    NSNotificationCenter *defaultNC = [NSNotificationCenter defaultCenter];
    [defaultNC postNotificationName:kFASGTMHTTPFetcherStoppedNotification
                             object:self];
  }
}
...
didReceiveAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
  @synchronized(self) {
    ...
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:challenge
                                                         forKey:kFASGTMHTTPFetcherErrorChallengeKey];
    NSError *error = [NSError errorWithDomain:kFASGTMHTTPFetcherErrorDomain
                                         code:kFASGTMHTTPFetcherErrorAuthenticationChallengeFailed
                                     userInfo:userInfo];
  ...
  @synchronized(self) {
    ...
    if (downloadFileHandle_ != nil) {
      ...
      @catch (NSException *exc) {
        ...
        NSError *error = [NSError errorWithDomain:kFASGTMHTTPFetcherStatusDomain
                                             code:kFASGTMHTTPFetcherErrorFileHandleException
                                         userInfo:userInfo];
        [self connection:connection didFailWithError:error];
        return;
      }
...
- (NSData *)cachedDataForStatus {
  if ([self statusCode] == kFASGTMHTTPFetcherStatusNotModified
  ...
  if (cachedData) {
    ...
    if (downloadFileHandle_ != nil) {
      ...
      @catch (NSException *) {
        ...
        status = kFASGTMHTTPFetcherErrorFileHandleException;
      }
    } else {
      ...
      if ([self shouldRetryNowForStatus:status error:nil]) {
        ...
        if ([downloadedData_ length] > 0) {
          userInfo = [NSDictionary dictionaryWithObject:downloadedData_
                                                 forKey:kFASGTMHTTPFetcherStatusDataKey];
        }
        error = [NSError errorWithDomain:kFASGTMHTTPFetcherStatusDomain
                                    code:status
                                userInfo:userInfo];
      }
    }
...
- (BOOL)isRetryError:(NSError *)error {
  ...
  struct retryRecord retries[] = {
    { kFASGTMHTTPFetcherStatusDomain, 408 }, // request timeout
    { kFASGTMHTTPFetcherStatusDomain, 503 }, // service unavailable
    { kFASGTMHTTPFetcherStatusDomain, 504 }, // request timeout
...
- (BOOL)shouldRetryNowForStatus:(NSInteger)status
  ...
  BOOL isFirstAuthError = (authorizer_ != nil)
    ...
    && (status == kFASGTMHTTPFetcherStatusUnauthorized); // 401
...
- (void)beginRetryTimer {
  ...
  [nc postNotificationName:kFASGTMHTTPFetcherRetryDelayStartedNotification
                    object:self];
}
...
- (void)destroyRetryTimer {
  ...
  if (shouldNotify) {
    NSNotificationCenter *defaultNC = [NSNotificationCenter defaultCenter];
    [defaultNC postNotificationName:kFASGTMHTTPFetcherRetryDelayStoppedNotification
                             object:self];
  }
}
...
- (void)setCookieStorageMethod:(NSInteger)method {
  ...
  if (method == kFASGTMHTTPFetcherCookieStorageMethodSystemDefault) {
    // System default
    ...
    if (method == kFASGTMHTTPFetcherCookieStorageMethodStatic) {
      // Store cookies in the static array
      NSAssert(gFASGTMFetcherStaticCookieStorage != nil,
               @"cookie storage requires FASGTMHTTPFetchHistory");

      self.cookieStorage = gFASGTMFetcherStaticCookieStorage;
    } else if (method == kFASGTMHTTPFetcherCookieStorageMethodFetchHistory) {
      // store cookies in the fetch history
      self.cookieStorage = [fetchHistory_ cookieStorage];
    } else {
      // kFASGTMHTTPFetcherCookieStorageMethodNone - ignore cookies
      self.cookieStorage = nil;
    }
  }
}
...
+ (id <FASGTMCookieStorageProtocol>)staticCookieStorage {
  return gFASGTMFetcherStaticCookieStorage;
}
...
- (id <FASGTMHTTPFetchHistoryProtocol>)fetchHistory {
  return fetchHistory_;
}

- (void)setFetchHistory:(id <FASGTMHTTPFetchHistoryProtocol>)fetchHistory {
  ...
  if (fetchHistory_ != nil) {
    ...
    [self setCookieStorageMethod:kFASGTMHTTPFetcherCookieStorageMethodFetchHistory];

  } else {
    // The fetch history was removed
    if (cookieStorageMethod_ == kFASGTMHTTPFetcherCookieStorageMethodFetchHistory) {
...
+ (Class)connectionClass {
  if (gFASGTMFetcherConnectionClass == nil) {
    gFASGTMFetcherConnectionClass = [NSURLConnection class];
  }
  return gFASGTMFetcherConnectionClass;
}

+ (void)setConnectionClass:(Class)theClass {
  gFASGTMFetcherConnectionClass = theClass;
}
...
NSString *FASGTMSystemVersionString(void) {
  ...
  if (savedSystemString == nil) {
    ...
    NSString *cleanedModel = FASGTMCleanedUserAgentString(model);
    ...
    if (uname(&unameRecord) == 0) {
      ...
      hardwareModel = FASGTMCleanedUserAgentString(machineName);
    } else {
      hardwareModel = @"unk";
    }
#endif
...
NSString *FASGTMApplicationIdentifier(NSBundle *bundle) {
  @synchronized([FASGTMHTTPFetcher class]) {
    ...
    identifier = [bundle objectForInfoDictionaryKey:@"FASGTMUserAgentID"];
    ...
    identifier = FASGTMCleanedUserAgentString(identifier);
    ...
    // Clean up whitespace and special characters
    version = FASGTMCleanedUserAgentString(version);
